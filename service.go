...
func (a *appService) Start(s service.Service) error {
	if service.Interactive() {
		preRun()
		log.Infof("starting kubemq bridges connectors version: %s", version)
	} else {
		logger.ServiceLog.SetLogger(a.svclogger)
		log.Infof("starting kubemq bridges connectors as a service version: %s", version)
	}
	go func() {
		err := runInteractive(a.serviceExit)
		if err != nil {
			log.Errorf("kubemq bridges ended with error: %s", err)
			os.Exit(1)
		} else {
			os.Exit(0)
		}
...
func (a *appService) Stop(s service.Service) error {
	a.serviceExit <- true
	log.Infof("kubemq bridges connectors stopped")
	return nil
}
