...
type Target interface {
	Init(ctx context.Context, connection config.Metadata, bindingName string, log *logger.Logger) error
	Do(ctx context.Context, request interface{}) (interface{}, error)
	Stop() error
}

func Init(ctx context.Context, kind string, connection config.Metadata, bindingName string, log *logger.Logger) (Target, error) {

	switch kind {
	case "target.command", "kubemq.command":
		target := command.New()
		if err := target.Init(ctx, connection, bindingName, log); err != nil {
			return nil, err
		}
		return target, nil
